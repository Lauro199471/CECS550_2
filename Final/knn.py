...
def data_no_nan(data):
    # Red Data
    red_Data = data[data[data.columns.tolist()[0]] ==
                    'red'][data.columns.tolist()[:]]
    red_Data.reset_index(drop=True, inplace=True)

    # get rid of label col
    red_Data_raw = red_Data.loc[:, "fixed acidity":"quality"]
    ...
    pd_red_Data_raw = pd.DataFrame(
        data=red_Data_raw, columns=data.columns.tolist()[1:])

    newRed = pd.concat(
        [red_Data[red_Data.columns.tolist()[0]], pd_red_Data_raw], axis=1)
    # ====================================================================================
    # White Data
    white_Data = data[data[data.columns.tolist()[0]] ==
                      'white'][data.columns.tolist()[:]]
    white_Data.reset_index(drop=True, inplace=True)

    # get rid of label col
    white_Data_raw = white_Data.loc[:, "fixed acidity":"quality"]
    ...
    pd_white_Data_raw = pd.DataFrame(
        data=white_Data_raw, columns=data.columns.tolist()[1:])

    newWhite = pd.concat(
        [white_Data[white_Data.columns.tolist()[0]], pd_white_Data_raw], axis=1)
...
class Knn(BaseEstimator):
    ...
    def predict(self, instance_data):
        ...
        for dt_pt in range(len(self.data)):
            ...
            dist = l2_distance(instance_data, instance2)
    ...
    def score(self, X, Y):
        ...
        for j in range(0, total):
            predicition = self.predict(X.iloc[j])
            if prediction == Y[j]:
                correct += 1
